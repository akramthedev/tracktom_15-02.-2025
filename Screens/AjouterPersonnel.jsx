...
export default function AjouterPersonnel({route}) {
 ...
 const [fontsLoaded] = useFonts({
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // Ajouter les données texte au FormData
            formData.append('name', fullName);
            formData.append('email', email);
            formData.append('password', password);
            formData.append('password_confirmation', password);
            formData.append('telephone', phone);
            formData.append('role_id', 1); // Par exemple, role_id est un champ texte
    
            if (imageUri) {
                const fileName = imageUri.split('/').pop();  
                const fileExtension = fileName.split('.').pop(); // Obtenir l'extension du fichier
    
                const image = {
                    uri: imageUri,
                    name: fileName,
                    type: `image/${fileExtension}`, // Exemple : 'image/jpeg' ou 'image/png'
                };
            ...
            const resp = await axios.post(`${ENDPOINT_API}staff`, formData, {
                headers: {
            ...
            if(e.status === 422){
                Alert.alert("Le mot de passe n'est pas trop fort.")
            }
        } finally {
            // setloading(false);
        }
    ...
    return (
        <>
            <PopUpNavigate setIsAuthenticated={setIsAuthenticated} isPopupVisible={isPopupVisible} setIsPopupVisible={setIsPopupVisible}  />
